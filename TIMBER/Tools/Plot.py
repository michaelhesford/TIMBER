...
def _doAxisTitles(h,main=True,split=False):
    ...
    if main:
        ...
        h.GetXaxis().SetTitleSize(0.15) #from 0.12
...
def CompareShapes(outfilename,year,prettyvarname,bkgs={},signals={},names={},colors={},
    ...
    if stackBkg:
        ...
        bkgStack.SetTitle(';%s;%s'%(prettyvarname if not doSoverB else '','A.U.')) #Avoid x axis label for "main" histogram if doing SoverB
         # Add bkgs to integral
        for bkey in bkgs.keys():
            tot_bkg_int += bkgs[bkey].Integral()
    else:
        for bkey in bkgs.keys():
            bkgs[bkey].SetTitle(';%s;%s'%(prettyvarname if not doSoverB else '','A.U.'))
    ...
    if len(bkgs.keys()) > 0:
	    ...
	    # First, draw background THStack and do axis labels
            ...
            if doSoverB:
                bkgStack.GetXaxis().SetLabelOffset(999) #effectively remove axis tick labels from "main" histogram when doing SoverB
    ...
    if doSoverB:
        ...
        s_over_b.GetXaxis().SetTitleOffset(1.1)
        s_over_b.SetTitle('')
        ...
        imax=1
        for m in maximums:
            if m != s_over_b.GetXaxis().GetXmin() and m != s_over_b.GetXaxis().GetXmax():
                ...
                mline.Draw('same')
                temp.append(mline)
                text = ROOT.TText(m,low+imax*(high-low)/3," %.2f "%m)
